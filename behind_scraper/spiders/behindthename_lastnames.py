...
def get_name(response):
    name = response.xpath(
        "//div[contains(@class, 'namemain') and contains(text(), 'Surname')]//text()"
    ).extract_first()
    if not name:
        return
    name = name.replace('Surname ', '').capitalize()
    return name


class BehindthenameLastnamesSpider(scrapy.Spider):
    name = 'behindthename_lastnames'
    allowed_domains = ['surnames.behindthename.com']
    start_urls = ['https://surnames.behindthename.com/names']
